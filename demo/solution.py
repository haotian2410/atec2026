...
class AlgSolution:
    ...
    def _compute_trash_servo_command(self, target):
        ...
        # scan 帧是历史朝向拍到的目标；机器人扫描结束时通常停在第三个视角，
        # 不能继续用旧图像的 image_error 当作当前误差，否则会朝错方向移动。
        # 因此 scan 目标先按 world_xy 做粗导航，等 live head/video 重新看到垃圾后
        # yolo_targets.py 的排序会自动切回图像伺服精调。
    ...
    def _select_near_sweep_target(self, targets):
        ...
        if body_near:
            self.trash_servo_mode = "body_track"
            self.body_target_lost_steps = 0
            return self._lock_or_choose(body_near, allow_switch_distance=0.25)
