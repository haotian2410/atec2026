...
class PiperKinematics:
    ...
    def ik(self, target_pos, target_R=None, q_init=None,
           iters=300, lam=0.05, step_clip=0.2, tol=1e-5):
        """逆运动学（阻尼最小二乘 DLS），在 base 系下求解。

        Parameters
        ----------
        target_pos : (3,)  base 系下的目标位置
        target_R   : (3,3) base 系下的目标姿态矩阵；为 None 时只约束位置（姿态自由）
        q_init     : (6,)  迭代初值（强烈建议传当前关节角，保证解连续、收敛快）
        iters      : 最大迭代次数（越大越精准）
        lam        : 阻尼系数（越大越稳但越慢，奇异点附近更平滑）
        tol        : 收敛阈值（误差范数）
        ...
        for _ in range(iters):
            M = self.fk(q)
            e_pos = np.asarray(target_pos, dtype=np.float64) - M[:3, 3]
            if use_ori:
                e = np.concatenate([e_pos, _rot_error(M[:3, :3], target_R)])
                J = self.jacobian(q)
            else:
                e = e_pos
                J = self.jacobian(q)[:3]
            if np.linalg.norm(e) < tol:
                break
            # DLS 更新量：dq = Jᵀ (J Jᵀ + λ²I)⁻¹ e
            JJt = J @ J.T
            dq = J.T @ np.linalg.solve(JJt + (lam ** 2) * np.eye(JJt.shape[0]), e)
            q = q + np.clip(dq, -step_clip, step_clip)      # 限制单步更新，防发散
            q = np.clip(q, self._Q_LOWER, self._Q_UPPER)    # 关节限位
