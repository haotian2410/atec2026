...
class TaskDRLEnvB2Cfg(TaskDEnvB2Cfg):  # 继承官方 TaskD B2Piper 环境，保留地图、箱子、终止和比赛基本逻辑。
    task_d_stage: str = "full"  # 训练阶段：climb/drop/push/full；不同阶段使用不同 reset 和奖励开关。
    ...
    def __post_init__(self):  # 环境配置初始化后处理。
        ...
        self.events.reset_task_d_stage = EventTerm(  # 用自定义 reset event 固定/课程化 robot 和 box 初始状态。
            ...
            params={"stage": self.task_d_stage, "robot_default_z": 0.8, "box_default_z": 0.5},  # 按当前 stage 设置 robot/box 位姿。
        )
