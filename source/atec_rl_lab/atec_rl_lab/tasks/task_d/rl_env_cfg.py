...
class TaskDRLEnvB2Cfg(TaskDEnvB2Cfg):  # 继承官方 TaskD B2Piper 环境，保留地图、箱子、终止和比赛基本逻辑。
    ...
    def __post_init__(self):  # 环境配置初始化后处理。
        ...
        if self.task_d_stage == "climb":  # 第一阶段：箱子已到位，只训练上箱子/上平台/越线。
            ...
            self.rewards.achieve.params["threshold"] = [2.0]  # drop reset 已经在 x>-1.4 后方，避免开局白拿第一段奖励。
            ...
            self.terminations.box_at_push_target = DoneTerm(  # 箱子到达垫高目标附近即成功结束，减少 box_step_target 原地刷分。
                ...
                params={"asset_cfg": SceneEntityCfg("box"), "target_xy": (-1.0, 1.6), "radius": 0.25},
                time_out=False,
            )
        elif self.task_d_stage == "mixed":  # 混合课程：每个 env reset 时采样 push/climb/drop/full 起点。
            self.rewards.achieve.params["threshold"] = [2.0]  # mixed 含 drop reset，避免 drop 子样本开局拿 x>-1.4 奖励。
...
class TaskDRLEnvB2PushCfg(TaskDRLEnvB2Cfg):  # Push 课程：箱子原位，机器人从箱子附近开始。
    task_d_stage: str = "push"  # 只训练把箱子推到 (-1.0, 1.6) 附近。
