...
class TaskDRLRewardsCfg:  # TaskD 专用训练奖励：比赛得分项 + 垫高上平台过程奖励 + 动作正则。
    ...
    box_progress = RewTerm(func=task_d_mdp.BoxForwardProgress, weight=30.0)  # 增量奖励箱子从初始 local x=0 向垫高目标 local x=2 移动；停住不再刷分。
    box_step_target = RewTerm(  # 奖励箱子接近指定垫高位置。
        func=task_d_mdp.BoxToStepTargetProgress,  # 计算箱子到固定 target_xy 的指数型距离奖励。
        params={"target_xy": (2.0, 1.6), "std": 0.75},  # target_xy 指定箱子垫高位置；std 控制奖励衰减范围。
    ...
    robot_on_box = RewTerm(  # 一次性奖励机器人踩到“已经推到目标附近”的箱子上，避免原地爬初始箱子刷分。
        ...
        params={"target_xy": (2.0, 1.6), "box_ready_radius": 0.25},
        weight=15.0,
    )
    robot_on_platform_side = RewTerm(  # 一次性奖励机器人到达高台/平台侧，避免停在平台边每步刷分。
        func=task_d_mdp.RobotOnPlatformSideOnce,
        params={"min_x": 2.8, "min_z": 1.35, "target_y": 1.6, "y_half_width": 1.4, "target_xy": (2.0, 1.6)},
        weight=30.0,
    )
...
class TaskDRLEnvB2Cfg(TaskDEnvB2Cfg):  # 继承官方 TaskD B2 环境，保留地图、箱子、终止和比赛基本逻辑。
    ...
    def __post_init__(self):  # 环境配置初始化后处理。
        ...
        self.scene.robot = UNITREE_B2_CFG.replace(
            ...
            init_state=UNITREE_B2_CFG.init_state.replace(pos=(-3, 0.0, 0.8)),
        )
        ...
        if self.task_d_stage == "climb":  # 第一阶段：箱子已到位，只训练上箱子/上平台/越线。
            ...
            self.terminations.box_at_push_target = DoneTerm(  # 箱子到达垫高目标附近即成功结束，减少 box_step_target 原地刷分。
                func=task_d_mdp.box_near_target_xy,
                params={"asset_cfg": SceneEntityCfg("box"), "target_xy": (2.0, 1.6), "radius": 0.25},
                time_out=False,
            )
...
class TaskDRLEnvB2PushCfg(TaskDRLEnvB2Cfg):  # Push 课程：箱子原位，机器人从箱子附近开始。
    task_d_stage: str = "push"  # 只训练把箱子推到 (2.0, 1.6) 附近。
