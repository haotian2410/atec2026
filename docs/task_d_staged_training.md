...
本文档用于 Unitree B2 Piper 的 Task D 固定地图过拟合策略：
...
`scripts/rsl_rl/train_task_d.py` 的默认行为保持不变：如果不传 `--task`，默认训练 `ATEC-TaskD-RL-B2Piper-Climb-v0`；如果存在 `atec_robot_model/baseline/unitree_b2_flat/policy.pt`，会自动作为弱预训练初始化。
...
恢复训练时，把 `<RUN_DIR>` 替换成 `logs/rsl_rl/task_d_b2_piper_box_step/` 下的具体 run 目录，把 `<CHECKPOINT>` 替换成类似 `model_2500.pt` 的 checkpoint 文件名。
...
`ATEC-TaskD-RL-B2Piper-v0` 也指向 Full 环境，但分阶段日志里建议显式使用 `ATEC-TaskD-RL-B2Piper-Full-v0`，方便区分阶段。
